...
class AccountManagement implements Taxable,Loanable{
    ...
    @Override
    public void calculateInterest(String taxAccount, double loan) {
        Account account = findAccount(taxAccount);
        if (account != null) {
            account.calcul(loan);
        }else {
            System.out.println("존재하지않는 계좌번호입니다 다시 입력해주세요");
        }
    }


    @Override
    public void calculateTax(String countNum) {
        Account account = findAccount(countNum);
        if (account != null) {
    ...
    public void run() {
        while (true) {
            ...
            System.out.println("5. 대출 이자 계산기");
            System.out.println("6. 세금 계산기");
            ...
            switch (choice) {
                ...
                case 5:  // 대출 이율 계산
                    System.out.print("계좌번호: ");
                    String taxAccount = scanner.nextLine();
                    System.out.print("대출받은 대출금을 입력해주세요: ");
                    double loan = scanner.nextInt();
                    calculateInterest(taxAccount,loan);
                    break;
                case 6: // 세금 계산기
                    System.out.print("계좌번호: ");
                    String countNum = scanner.nextLine();
                    calculateTax(countNum);
                default:
                    break;
            }
        }
